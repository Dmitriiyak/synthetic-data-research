...
![Domain Gap](results/domain_gap.png)

На данном графике представлена разница между точностью модели на синтетической и реальной тестовых выборках (Domain Gap). Положительное значение означает, что модель показывает более высокую точность на синтетических изображениях по сравнению с реальными.

Во всех пяти экспериментах разница остаётся положительной, что свидетельствует о систематическом завышении оценки качества модели при использовании синтетических тестовых данных. Наибольший разрыв наблюдается в эксперименте **E5**, где модель обучалась исключительно на синтетических изображениях.

В эксперименте **E5** величина Domain Gap превышает **10 процентных пунктов**, что свидетельствует о значительном расхождении между результатами оценки на синтетической и реальной тестовых выборках. Полученные результаты показывают, что синтетические изображения могут быть полезны для обучения моделей, однако их использование в качестве единственной тестовой выборки способно привести к существенной переоценке качества модели.

## Итоги исследования

### 1. Можно ли обучать модель на синтетических данных?

Исследование показало, что использование синтетических данных может не ухудшать точность модели при соблюдении определённых пропорций между реальными и синтетическими изображениями. При этом полная замена реальных данных синтетическими приводит к заметному снижению качества модели. Поэтому при недостаточном количестве реальных изображений для обучения модели классификации использование синтетических данных может быть эффективным решением.

### 2. До какого уровня это эффективно?

Проведённые эксперименты показали, что использование до **75% синтетических изображений** в обучающей выборке практически не ухудшает качество модели. Поэтому для эффективного применения синтетических данных не рекомендуется превышать долю **75%** без дополнительной проверки на других наборах данных и моделях.

### 3. Можно ли использовать синтетические данные для оценки?

Исследование показало, что при оценке модели на синтетических изображениях точность систематически оказывается выше, чем при оценке на реальных данных. Это означает, что использование только синтетических тестовых выборок не позволяет объективно оценить качество модели и может привести к существенной переоценке её возможностей.

### 4. Практическая значимость исследования

Полученные результаты помогают решить проблему недостаточного количества данных при обучении моделей классификации изображений. Исследование показывает, что синтетические изображения могут успешно использоваться для расширения обучающих выборок при ограниченном количестве реальных данных. В то же время работа демонстрирует, что использовать синтетические изображения в качестве единственной тестовой выборки не рекомендуется, поскольку это не позволяет получить объективную оценку качества модели.
